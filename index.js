    ...
    async handle(handlerInput) {
        ...
        let speechText;
         if(sessionAttributes['name']){
            ...
            speechText = requestAttributes.t('WELCOME_MSG',  name);
...
const RegisterPersonCallIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const nombre = intent.slots.nom_llamada.value;
        
        if(!nombre){//Si no tengo el nombre guardado, lo debo de guardar
            
            sessionAttributes['nombreLlamada'] = nombre;
        
            sessionAttributes['dias_sin_hablar'] = moment().tz(timezone).startOf('day');
            
            return handlerInput.responseBuilder//aqui digo que lo he registrado y cierro
                .speak(handlerInput.t('NOMBRE_REGISTRADO_MSG'))
                .getResponse();
            ...
            return handlerInput.responseBuilder
                    .speak(handlerInput.t('DIAS_SIN_HABLAR_MSG'), daysLeft,nombre,nombre)
                    .reprompt(handlerInput.t('DIAS_SIN_HABLAR_MSG'), daysLeft,nombre,nombre)
                    .getResponse();
        }
...
/*const SayDaysWithoutCallIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        if(!diaLlamada){
            ...
            intent.slots.dias_sin_hablar.value=moment().tz(timezone).startOf('day');
        
            if(intent.confirmationStatus !== 'CONFIRMED') {

                return handlerInput.responseBuilder
                    .speak(handlerInput.t('REPETIR_NOMBRE_MSG'))
                    .reprompt(handlerInput.t('REPETIR_NOMBRE_MSG'))
                    .getResponse();
            }
            
            return handlerInput.responseBuilder//aqui digo que lo he registrado y cierro
                .speak(handlerInput.t('NOMBRE_REGISTRADO_MSG'), nombre)
                .getResponse();
            
        }else{
            const today=moment().tz(timezone).startOf('day');//dia actual
            
            const daysLeft = diaLlamada.startOf('day').diff(today, 'days');//dias que han pasado desde el primer dia hasta el actual
        ...
        //const daysLeft = nextBirthday.startOf('day').diff(today, 'days');
